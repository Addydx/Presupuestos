...
class PdfService {
      ...
      pw.MultiPage(
        ...
        build: (ctx) => [
          ...
          if (manoObra.isNotEmpty) ...[
            _seccionTitulo('Mano de Obra', const PdfColor.fromInt(0xFF7B1FA2)),
            pw.SizedBox(height: 6),
          ...
          if (equipos.isNotEmpty) ...[
            _seccionTitulo('Equipos Rentados', const PdfColor.fromInt(0xFFE65100)),
            pw.SizedBox(height: 6),
  ...
  pw.Widget _tablaMateriales(List<MaterialPresupuesto> materiales) {
    const encabezados = ['Nombre', 'Categoría', 'Unidad', 'Cantidad', 'P. Unit.', 'Total'];
    final filas = materiales.map((m) => [
          m.nombre,
          m.categoria,
          m.unidad,
  ...
  pw.Widget _tablaManoObra(List<ManoObra> manoObra) {
    ...
    final filas = manoObra.map((mo) {
      final tipoPago = mo.tipoPago == TipoPago.porDia ? 'Por Día' : 'Contrato';
      final detalle = mo.tipoPago == TipoPago.porDia
  ...
  pw.Widget _tablaFinanzas(Map<String, double> r, Finanzas finanzas) {
    final filas = [
      ['Costo Directo (Materiales + M.O. + Equipos)', '\$${r['costoDirecto']!.toStringAsFixed(2)}'],
      ['Imprevistos (${finanzas.porcentajeImprevistos.toStringAsFixed(1)}%)', '\$${r['imprevistos']!.toStringAsFixed(2)}'],
      ['Subtotal', '\$${r['subtotal']!.toStringAsFixed(2)}'],
      ['Utilidad (${finanzas.porcentajeUtilidad.toStringAsFixed(1)}%)', '\$${r['utilidad']!.toStringAsFixed(2)}'],
